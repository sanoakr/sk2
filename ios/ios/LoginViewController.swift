...
//		useridField.bounds.insetBy(dx: 10, dy: 10)
	...
	@IBAction func login(_ sender: UIButton) {
        ...
        ServerAuth.connect(serverAddress: serverAddress, serverPort: serverPort)
		
		let sendtext = "AUTH,\(String(describing: useridField.text!)),\(String(describing: passwordField.text!))"
		
		_ = ServerAuth.sendCommand(command: sendtext)
		let retval = ServerAuth.sendCommand(command: "end")
...
//        print("---------------------> \(readVal_decoded)")  // デバッグ
        ...
        // 認証失敗
        if readVal_decoded.contains("authfail") {
        ...
        // 認証成功
            ...
            if readVal_decoded.hasSuffix("]") {

                let splitRead = readVal_decoded.components(separatedBy: ",")

                let key:String = splitRead[0]
                let engName:String = splitRead[1]
                let jpnName:String = splitRead[2]

                
                let splitRead2 = readVal_decoded.components(separatedBy: ",[")
